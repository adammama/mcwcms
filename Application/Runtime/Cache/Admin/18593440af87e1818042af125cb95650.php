<?php if (!defined('THINK_PATH')) exit();?><div class="bjui-pageContent">
    <form action="<?php echo U('useredit');?>" class="pageForm" data-toggle="validate">
		<input type="hidden" name="id" value="<?php echo ($userone["id"]); ?>">
        <div class="pageFormContent" data-layout-h="0">
          <table class="table table-condensed table-hover" width="100%">
           <tbody>
			<tr>
				<td><label for='username' class='control-label x85'>用户:</label>
					<input type='text' id='username' name='username' data-rule='required;' size='20' value="<?php echo ($userone["username"]); ?>"></td>
			</tr>
			<tr>
			...
			<tr>
				<td><label for='status' class='control-label x85'>状态:</label>
					<select name="status" data-toggle="selectpicker">
						<?php if($userone["status"] > 0 ): ?><option value="1">激活</option>
			...
			<tr>
				<td><label for='group_input' class='control-label x85'>所属角色:</label>
					<select name="group_id" data-toggle="selectpicker">
						<?php if(is_array($role)): foreach($role as $key=>$v): if($v['id'] == $userone['group_id']): ?><option value="<?php echo ($v["id"]); ?>" selected><?php echo ($v["title"]); ?></option>
			            	<?php else: ?>
			...
			<tr><td colspan=2><label for='remark' class='control-label x85'>备注:</label>
				<textarea name='remark' cols='65' rows='5' ><?php echo ($userone["remark"]); ?></textarea></td></tr>
			<tr><td></td></tr>
           </tbody>
          </table>
        </div>
